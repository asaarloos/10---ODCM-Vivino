...
#wine_data needs to be replaced with the name of the final dataset
# Replace commas with periods in the Rating column
wine_data$Rating <- gsub(",", ".", wine_data$Rating)

# Remove the euro sign and replace commas with periods in the Price column
wine_data$Price <- gsub("€", "", wine_data$Price)  
wine_data$Price <- gsub(",", ".", wine_data$Price) 

# Convert Rating and Price columns to numeric
wine_data$Rating <- as.numeric(wine_data$Rating)
